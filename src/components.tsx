...
export function QuestionItem(props: QuestionItemProps) {
  const {
    questionItem: initialQuestionItem,
  ...
  const prevQuestionItem: QuestionnaireItem | undefined =
    usePreviousValue(questionItem);
  ...
  if (isGroupItem(questionItem, context)) {
    if (itemControl) {
      if (
        ...
        console.warn(`QRF: Unsupported group itemControl '${itemControl.coding![0]!
          .code!}'. 
                Please define 'itemControlGroupWidgets' for '${itemControl.coding![0]!
                  .code!}'`);
    ...
    if (!groupItemComponent) {
      console.warn(
        `QRF: groupWidget is not specified but used in questionnaire.`
      );
...
export function QuestionnaireResponseFormProvider({
  children,
  ...props
}: PropsWithChildren<QRFContextData>) {
...
export function evaluateQuestionItemExpression(
  ...
  try {
    ...
    throw Error(
      `FHIRPath expression evaluation failure for ${linkId}.${path}: ${err}`
    );
  }
}
