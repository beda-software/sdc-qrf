...
export function getBranchItems(
  ...
  for (let i = 0; i < fieldPath.length; i++) {
    qItem = qItem.item!.find(
      (curItem: any) => curItem.linkId === fieldPath[i]
    )!;
...
export function compareValue(
  firstAnswerValue: AnswerValue,
  secondAnswerValue: AnswerValue
) {
...
function mapFormToResponseRecursive(
  ...
  return Object.entries(answersItems).reduce((acc, [linkId, answers]) => {
    ...
    const question = questionnaireItems.filter(
...
function initialToQuestionnaireResponseItemAnswer(
  initial: QuestionnaireItemInitial[] | undefined
) {
  return (initial ?? []).map(
    ({ value }) => ({ value } as QuestionnaireResponseItemAnswer)
  );
}
    ...
    return (values, answerValue) => {
      const answersLength = _.reject(
        ...
        (value) =>
          ...
          _.every(_.mapValues(value.value, isValueEmpty))
      ).length;
...
function isQuestionEnabled(args: IsQuestionEnabledArgs) {
  ...
  if (
    ...
    enableWhenExpression.language === "text/fhirpath"
  ) {
    try {
  ...
  return iterFn(enableWhen, ({ question, answer, operator }) => {
    ...
    if (_.includes(args.parentPath, question)) {
      ...
      const parentAnswerPath = _.slice(
        args.parentPath,
        0,
        args.parentPath.length - 1
      );
...
function removeDisabledAnswersRecursive(
  args: RemoveDisabledAnswersRecursiveArgs
): FormItems {
...
function resolveTemplateExpr(str: string, context: ItemContext) {
  ...
  if (matches) {
    return matches.reduce((result, match) => {
      ...
      const resolvedVar = fhirpath.evaluate(
        context.context || {},
        expr,
        context,
        undefined,
